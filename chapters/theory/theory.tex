...
to as a \textit{perceptron} in literature. The simples network structure contains a
single layer of N such nodes, and is most often called a \textit{single-layer perceptron}.
Adding an additional layer of nodes, a so-called \textit{hidden layer}, results in a
type of feed-forward neural network (FFNN), typically referred to as a Multilayer Perceptron (MLP)
(see figure \ref{fig:theory-mlp}).
...
$\mathrm{W}_l \bs{y}_{l-1}$ we move forward one layer.
