...
and have applications in medicine and industry.

\begin{itemize}
	\item Low-energy nuclear physics
	\item Scintillator
	\item Challenges with the data
	\item Current methods for fitting
	\item 
\end{itemize}
...
\begin{itemize}
	...
	\item All information in the detector is timestamped sub nanoseconds.
	\item Total energy deposited in detector is recorded
\end{itemize}

\subsection{Decay Spectroscopy}
...
\end{itemize}
of which this experiment focuses on Beta decay, photon decay and Internal
...
\end{equation}
This will give a set of possible energies, as the emitted electron can come from
different shells in the atom.

Gamma ray decay leads to a characteristic energy representative of the difference
...
in the scintillator. The central detector observes ions and decays, of which decays can
...
\begin{itemize}
	...
	\item Some characteristic time later a decay is detected. Position and time of decay
	are recorded.
	...
	particular ion
\end{itemize}
The ability to correlate decays to specific ions is dependent on the rate of ions implanted.
Low-energy photons will be trapped in the detector, while high-energy photons will mostly
escape. This cutoff happens around 100keV. Electrons are typicly on the order of a few
MeV. Because you need a fairly large amount of material to completely stop an electron with
energies of 5-10MeV, what is actually detected is the energy left in the detector by the
electron as it passes through.
Simulated data generated using Geant4 \cite{Agostinelli2003} with what is hopefully realistic detector inputs.
The experimental data is take from a recent beta-decay experiment.
Gaps in distribution experimentally due to pixels not responding.
